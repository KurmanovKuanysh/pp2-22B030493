    ...
    while check:
        ...
        if n == 2:
            ...
            try:
                ...
                if type(a) is int:
                    cur.execute(
                        f"SELECT * FROM phonebook WHERE phone = '{np}'")
                    cnt = cur.fetchall()
                    if len(cnt) > 1:
                        cur.execute(
                            f"SELECT * FROM phonebook WHERE phone = '{np}'")
                        ...
                        print(cnt)
                        np = input()
                        # Use CALL statement and cur.execute()
                        cur.execute(f"CALL delete_phone('{np}')")
                    if len(cnt) == 1:
                        # Use CALL statement and cur.execute()
                        cur.execute(f"CALL delete_phone('{np}')")
            except ValueError:
        ...
        if n == 5:
            ...
            start_p = int(input())
            cur.execute("SELECT * FROM get_paginated_data(%s, %s)",
                        (page, start_p))
            result = cur.fetchall()
            for i in result:
                print(i)
